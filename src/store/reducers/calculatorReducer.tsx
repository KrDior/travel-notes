...
export const calculatorReducer = handleActions<State, CalculatorModel>(
  {
    [CalculatorActions.Type.SET_OPERANDA]: (state: State, action: Action<OperandAPayload>) => {
      return {
        operandA: action.payload ? (action.payload as any) : 0,
        operandB: state.operandB,
      };
    },
    [CalculatorActions.Type.SET_OPERANDB]: (state: State, action: Action<OperandBPayload>) => {
      return {
